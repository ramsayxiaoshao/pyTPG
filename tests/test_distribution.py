...
def test_installed_wheel(tmp_path: Path) -> None:
    ...
    expected_version = version("pytpg")
    dist_info = f"pytpg-{expected_version}.dist-info"
    with ZipFile(wheel) as archive:
        names = archive.namelist()
        assert "pytpg/__init__.py" in names
        assert "pytpg/py.typed" in names
        assert all(
            name.startswith(("pytpg/", f"{dist_info}/"))
            for name in names
        )
        assert not any(
        ...
        for filename in ("LICENSE",):
            ...
            assert f"{dist_info}/licenses/{filename}" in names
...
assert pytpg.__version__ == importlib.metadata.version('pytpg') == sys.argv[2]
...
""", str(installed), expected_version],
